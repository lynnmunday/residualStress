...
[Mesh]
  ...
  [cut]
    ...
    combinatorial_geometry = 'x>${fparse -cutHalfWidth} & x<${cutHalfWidth} & y>${fparse height-totalCut}'
    block_id = 99
  []
  [del]
  ...
  [cutFaceLeft]
    type = ParsedGenerateSideset
    combinatorial_geometry = 'x>${fparse -cutHalfWidth} & x<0 & y>${fparse height-totalCut} & y<${fparse height-totalCut+cutIncrement}'
    normal = '1 0 0'
    new_sideset_name = cutFaceLeft
    input = cutFaceRight
  []
  [cutFaceTop]
    type = ParsedGenerateSideset
    combinatorial_geometry = 'x>${fparse -cutHalfWidth} & x<${cutHalfWidth} & y>${fparse height-totalCut} & y<${fparse height-totalCut+cutIncrement}'
    normal = '0 1 0'
...
[Outputs]
  file_base = 'cut_${totalCut}_outputs/homoForward'
  csv = true
[]
...
[Functions]
  [cutFaceRight_function]
    ...
    param_vector_name = 'params_cutFace/vals'
  []
  [cutFaceLeft_function]
    type = ParsedOptimizationFunction
    expression = '-a'
    param_symbol_names = 'a'
    param_vector_name = 'params_cutFace/vals'
...
[BCs]
  ...
  [cutFaceLeft]
    type = FunctionNeumannBC
    variable = disp_x
    boundary = cutFaceLeft
    function = cutFaceLeft_function
  []
[]
