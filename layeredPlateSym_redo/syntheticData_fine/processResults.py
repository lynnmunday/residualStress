...
df_noCut = pd.read_csv('cut_0.0e-3_outputs/results_dispy_top_0001.csv')
weight=1e9
cut_depth=np.arange(0.1,1.3,0.1)
for cut in cut_depth:
    df_all_output = pd.DataFrame()
    ...
    results_dir='cut_{:.2f}e-3_outputs/'.format(cut)
    #post-process measurement data with weights
    name=results_dir+'results_dispy_top_0001.csv'
...
fig=plt.figure()
plt.plot(df_filter['id'],df_filter['weighted_diff_disp_10noise'],'-',color='y',\
             linewidth=1,marker='*',label='noise=10')
plt.plot(df_filter['id'],df_filter['weighted_diff_disp_5noise'],'-',color='m',\
             linewidth=1,marker='*',label='noise=5')
plt.plot(df_filter['id'],df_filter['weighted_diff_disp_1noise'],'-',color='c',\
             linewidth=1,marker='*',markersize=4,label='noise=1')
plt.plot(df_filter['id'],df_filter['weighted_diff_disp'],'-',color='k',\
             linewidth=1,marker='*',markersize=2,label='noise=0')
plt.legend(loc='best', ncol=1)
